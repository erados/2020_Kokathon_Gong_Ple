...
class Calendar extends Component<Props> {
    ...
    onRowClicked() {
        ...
        return console.log(this.state.hide);
    }
    ...
    createCalendar() {
        ...
        for (let i = firstDay - offset + 1; i < lastDay; i += 7) {
            let temp = [];
            for (let j = i; j < i + 7; j++)
                temp.push(<IonCol className={(j < today && j > 0) ?((this.props.success[j-1] === 1) ? "success" : "failure"):""}>{(j > 0) ? ((j <= lastDay) ?  j : "") : ""}{((j == today) ? ( <br/>) : "")}{((j == today) ?<span className={"today"}>TODAY</span> : "")}</IonCol>);
            children.push(<IonRow onClick={this.onRowClicked.bind(this)}>{temp}</IonRow>);
        }
        children.push(<IonList style={{display: this.state.hide ? 'block' : 'none'}}>
